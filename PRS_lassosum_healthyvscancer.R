...
# Load the covariate file for healthy vs cancer.
cov                 <- fread(paste0(bfile, ".eigenvec"))
covariate           <- as.data.frame(cov)
colnames(covariate) <- c('FID', 'IID', 'PC1', 'PC2')
...
write.table(lass_res, 
            'lass_lambda_selection.txt', 
            quote     = F, 
            row.names = F, 
            col.names = T, 
            sep       = '\t')
